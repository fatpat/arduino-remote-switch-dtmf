...
int DTMF::ProcessSample(int sample,int adc_centre)
{
  float Q0;
...
long DTMF::detect(float dtmf_mag[],int adc_centre)
{
  int index;
  float d_tmp;
  long ret = 0;
  ...
  {
    ret += ProcessSample(testData[index],adc_centre);
  }
